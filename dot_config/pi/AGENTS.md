...
If you are not already in a git worktree, or if the change isn't related to the worktree you're in, create a new one with a descriptive branch name before making code changes. Use the `/skill:worktree <branch-name>` skill to create isolated workspaces (e.g., `/skill:worktree feature/add-auth`, `/skill:worktree fix/login-bug`).
